...
def main():
    ...
    st.markdown("EPDK lisans verileri ile sözleşme bitiş takibi, risk analizi ve pazar dağılımı.")
    ...
    if uploaded_file:
        try:
            ...
            if target_date_col not in df.columns:
                st.error("Hata: 'Dağıtıcı ile Yapılan Sözleşme Bitiş Tarihi' sütunu bulunamadı.")
                st.stop()
            ...
            # ==============================================
            with tab_risk:
                # 1. ACİL DURUM TABLOSU (Tam Genişlik)
                ...
                with col_g1:
                    ...
                    st.plotly_chart(
                        px.bar(y_data, x='Yıl', y='Adet', text='Adet', color='Adet', color_continuous_scale='Reds'),
                        use_container_width=True)
            ...
            with tab_harita:
                ...
                with c_map:
                    ...
                    if not map_data.empty:
                        fig_m = px.scatter_mapbox(map_data, lat='lat', lon='lon', color='Dağıtım Şirketi',
                                                  hover_name='Unvan', zoom=5, height=600, mapbox_style="carto-positron")
                ...
                with c_heat:
                    ...
                    st.plotly_chart(
                        px.density_heatmap(heat_d, x='İl', y='Dağıtım Şirketi', color_continuous_scale='Blues'),
                        use_container_width=True)

            # ==============================================
            # TAB 3: PAZAR PAYI (PASTA DİLİMİ DÜZELTİLDİ)
            # ==============================================
            with tab_pazar:
                # Sütunları eşit bölelim ki grafik sıkışmasın
                col_tree, col_pie = st.columns(2)

                with col_tree:
                    st.subheader("🏢 Marka Hiyerarşisi (Treemap)")
                    fig_tree = px.treemap(df_filtered, path=['Dağıtım Şirketi', 'İl'])
                    st.plotly_chart(fig_tree, use_container_width=True)

                with col_pie:
                    st.subheader("🍰 Pazar Payı (Pasta Grafiği)")

                    # Veriyi güvenli şekilde hazırlayalım
                    pie_source = df_filtered['Dağıtım Şirketi'].value_counts().reset_index()
                    pie_source.columns = ['Şirket', 'Adet']  # İsimleri zorla ata

                    # Çok fazla şirket varsa grafik bozulur, ilk 12'yi alıp kalana "Diğer" diyelim
                    if len(pie_source) > 12:
                        top_12 = pie_source.iloc[:12]
                        other_val = pie_source.iloc[12:]['Adet'].sum()

                        # Yeni satır oluştur (Pandas concat ile güvenli ekleme)
                    ...
                    # Legend (açıklama) grafiği sıkıştırmasın diye alta alabiliriz veya varsayılan bırakabiliriz
            ...
            # ==============================================
            with tab_veri:
                st.subheader("📋 İndirilebilir Veri Listesi")

                export_df = df_filtered[
                    ['Lisans No', 'Unvan', 'İl', 'İlçe', 'Dağıtım Şirketi', target_date_col, 'Kalan_Gun',
                     'Risk_Durumu']].sort_values('Kalan_Gun')
                export_df[target_date_col] = export_df[target_date_col].dt.strftime('%Y-%m-%d')

                st.download_button("📥 Excel/CSV Olarak İndir", export_df.to_csv(index=False).encode('utf-8'),
                                   "rapor.csv", "text/csv")
                ...
                st.dataframe(export_df.style.applymap(color_row, subset=['Risk_Durumu']), use_container_width=True,
                             height=600)

        except Exception as e:
            st.error(f"Bir hata oluştu: {e}")
            st.info("Lütfen Excel dosyanızın sütun isimlerini kontrol ediniz.")
    else:
        st.info("👈 Başlamak için lütfen sol menüden Excel dosyanızı yükleyiniz.")
